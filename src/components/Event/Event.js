...
const scrubbedColumnTitleMap = {
	lastName: "Scrubbed Last Name",
	firstName: "Scrubbed First Name",
	gender: "Scrubbed Gender",
	club: "Scrubbed Club",
	birthDate: "Scrubbed Birth Date",
	usssLicense: "Scrubbed USSS License",
	usssSprintPoints: "Scrubbed USSS Sprint Points",
	usssDistancePoints: "Scrubbed USSS Distance Points",
	division: "Scrubbed Division",
	fisLicense: "Scrubbed FIS License",
	fisSprintPoints: "Scrubbed FIS Sprint Points",
	fisDistancePoints: "Scrubbed FIS Distance Points",
	country: "Scrubbed Country",
...
export default function Event(props) {
	...
	const getRegData = useCallback(() => {
		firestore
			...
			.then((doc) => {
				...
				const mappedScrubbedData = _.map(doc.data().scrubbedData, (row) => {
					return {
						...
						usssSprintPoints: _.get(row, "usssSprintPoints", ""),
						usssDistancePoints: _.get(row, "usssDistancePoints", ""),
						division: _.get(row, USSS.DIVISION, ""),
						fisLicense: _.get(row, "fisLicense", ""),
						fisSprintPoints: _.get(row, "fisSprintPoints", ""),
						fisDistancePoints: _.get(row, "fisDistancePoints", ""),
						country: row.country,
						error: row.error,
						id: row.id,
					};
				});
				mappedScrubbedData[0] = scrubbedColumnTitleMap;
				setRows(mappedScrubbedData);
				setSaveRows(mappedScrubbedData);
				...
				const mappedRegData = doc.data().regData.map((reg) => {
					let row = [];
					keys.forEach((key) => {
						row.push(reg[key] || "");
					});
	...
	useEffect(() => {
		...
		firestore
			...
			.catch((err) =>
				console.log("Error getting USSS Women's Points List: ", err)
			);
		...
		firestore
			...
			.get()
			.then((doc) => {
				const pointsList = _.concat(
					womensPointsList,
					_.slice(doc.data().pointsList, 1)
				);
				setUsssPointsList(pointsList);
			})
			.catch((err) =>
				console.log("Error getting USSS Men's Points List: ", err)
			);
		...
		firestore
			...
			.catch((err) =>
				console.log("Error getting FIS Women's Points List: ", err)
			);
		...
		firestore
			...
			.catch((err) =>
				console.log("Error getting FIS Men's Points List: ", err)
			);
	}, []);
	...
	const handleChange = (select, key) => {
		...
		const rows = _.map(regData, (row) => {
			...
			rowValues = {
				...rowValues,
				...{ [colMap[0].columnName]: scrubbedColumnTitleMap[key] },
			};
			for (let i = 1; i < colMap.length; i++) {
				rowValues = {
					...rowValues,
					...{ [colMap[i].columnName]: row[colMap[i].columnId] },
				}; //merge the columns in to the scrubbed object array
			}
		...
		firestore
			.collection(auth.currentUser.uid)
			.doc(eventId)
			.update({ columnMap, scrubbedData: rows })
			.catch((error) => {
	...
	const handleExportClick = () => {
		const expFile = saveRows.map((row, idx) => {
			return Object.values(row).concat(Object.values(regData[row.id]));
		});
		const exportWorksheet = XLSX.utils.aoa_to_sheet(expFile);
		...
		XLSX.writeFile(exportWorkbook, "scrubbed_data.xlsx");
	};
	...
	const handleScrubClick = () => {
		...
		firestore
			.collection(auth.currentUser.uid)
			.doc(eventId)
			.update({ scrubbedData: scrubbedRows })
			.catch((error) => {
				console.error(`Error updating document ${eventId}:`, error);
			});
	};
	...
	const verify = (listType, pointsList, row, result) => {
		const predicate =
			listType === "USSS"
				? [USSS.USSA_ID, row.usssLicense]
				: [FIS.FIS_CODE, row.fisLicense];
	...
	const verifyName = (regData, pointsData, listType) => {
		...
		let error =
			regData.firstName.toLowerCase() !==
			pointsData[type.FIRST_NAME].toLowerCase()
				? `${type.NAME} First Names do not match. `
				: "";
		error +=
			regData.lastName.toLowerCase() !==
			pointsData[type.LAST_NAME].toLowerCase()
				? `${type.NAME} Last Names do not match. `
				: "";
		error +=
			moment(regData.birthDate.toDate()).format("YYYY") !==
			pointsData[type.YOB].toString()
				? `${type.NAME} Birth Years do not match. `
				: "";
		error +=
			regData.gender !== pointsData[type.GENDER]
				? `${type.NAME} Genders do not match. `
				: "";
		return error;
	};
	...
	const filterLastName = (listType, pointsList, lastName) => {
		const type = listType === "USSS" ? USSS : FIS;
		const racers = _.filter(
			pointsList,
			(i) =>
				type.LAST_NAME in i &&
				i[type.LAST_NAME].toLowerCase() === lastName.toLowerCase()
		);
		return racers.length > 0
			? `${racers.length} racers found with last name of ${lastName} on ${listType} Points List. `
			: "";
	};
			...
			{menuItems.length > 0 ? (
				<div>
					<SimpleSelect
						...
						value={_.get(
							_.find(columnMap, (o) => o.columnName === "lastName"),
							"columnId",
							""
						)}
					/>
					<SimpleSelect
						...
						value={_.get(
							_.find(columnMap, (o) => o.columnName === "firstName"),
							"columnId",
							""
						)}
					/>
					<SimpleSelect
						...
						value={_.get(
							_.find(columnMap, (o) => o.columnName === "gender"),
							"columnId",
							""
						)}
					/>
					<SimpleSelect
						...
						value={_.get(
							_.find(columnMap, (o) => o.columnName === "birthDate"),
							"columnId",
							""
						)}
					/>
					<SimpleSelect
						...
						value={_.get(
							_.find(columnMap, (o) => o.columnName === "club"),
							"columnId",
							""
						)}
					/>
					<SimpleSelect
						...
						value={_.get(
							_.find(columnMap, (o) => o.columnName === "usssLicense"),
							"columnId",
							""
						)}
					/>
					<SimpleSelect
						...
						value={_.get(
							_.find(columnMap, (o) => o.columnName === "fisLicense"),
							"columnId",
							""
						)}
					/>
